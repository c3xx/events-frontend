<script lang="ts">
	...
	let columns: TableProps<OrganizationMember>['columns'] = [
		{
			key: 'fullName',
			header: 'Name',
			type: 'button',
	...
	const optionsList: TableProps<OrganizationMember>['optionsList'] = [
		{
			id: '1',
			name: 'Edit Role(s)',
			onclick: (row) => {
	...
	let selectedMember: null | OrganizationMember = $state(null);

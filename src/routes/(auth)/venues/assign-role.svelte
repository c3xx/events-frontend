	...
	<Sheet.Content class="h-screen w-full sm:min-w-120" side="right">
		...
		>
			<Sheet.Header>
				...
				<Sheet.Description>
					Select a user and a role to add them to {venueAssignState.selectedVenue?.name ??
						'the venue'}.
				</Sheet.Description>
			</Sheet.Header>
			<div class="mx-xs flex h-full flex-col overflow-auto py-4">
				{#if errorText}
					<p class="mb-4 text-sm text-red-500">{errorText}</p>
				{/if}
				{#if successText}
					<p class="mb-4 text-sm text-green-500">{successText}</p>
				{/if}

				<div class="sticky bottom-0 z-10 flex flex-col pt-4">
					<Separator class="my-xs" />
					<div
						class="flex items-center justify-center gap-2 rounded-lg bg-secondary p-xs max-sm:flex-col"
					>
						...
						<div class="grid w-full gap-3">
							<DynamicSelectButton
								...
								loadFn={() =>
									venueAssignState.selectedVenue
										? loadRolesVenueType(venueAssignState.selectedVenue.venueTypeId)
										: Promise.resolve([])}
								mapOption={(item: { id: String; name: string }) => ({
